...
That falls out of what a cluster label already means, rather than being a display mode bolted on top. A point labelled `-1` is drawn dark grey **and** refused by the viewer's picking filters. Unclaimed points are `-1` — which is why, before this, they could not be picked at all and the workflow only functioned with the input cloud shown alongside the result, drawing every point twice.
