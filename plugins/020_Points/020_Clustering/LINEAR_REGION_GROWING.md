...
- **Offer only the points ahead** — the candidates are exactly the unclaimed points the panel counts as lying ahead: the corridor the march itself would have searched, about 9 m long and 0.6 m wide at the defaults (`_RANK_REACH_FACTOR` / `_RANK_RADIUS_FACTOR` in the grower).
- Untick it to offer *every* unclaimed point in the cloud instead — for the rare continuation that lies outside the corridor entirely.
