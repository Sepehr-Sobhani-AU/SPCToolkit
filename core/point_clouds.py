...
"""
Module for managing point cloud data within the application.
...
by unique identifiers, as well as tracking each point cloud's metadata.
...
class PointClouds(QObject):
    ...
    # Signal to notify about changes in the point clouds
    point_clouds_updated = pyqtSignal(dict)

    def __init__(self):
        """Initializes the manager with an empty dictionary for storing point clouds."""
        super().__init__()
        self._point_clouds = {}  # Dictionary to store point cloud data by unique identifier
        self._previous_keys = set()  # Store the previous keys for comparison

    # def __setitem__(self, key: uuid.UUID, value: PointCloud):
    #     """
    #     Adds or updates a point cloud and triggers notifications.
    #     """
    #     self._point_clouds[key] = value
    #     self._notify_changes()
    #
    # def __delitem__(self, key: uuid.UUID):
    #     """
    #     Deletes a point cloud by its key and triggers notifications.
    #     """
    #     if key in self._point_clouds:
    #         del self._point_clouds[key]
    #         self._notify_changes()
    ...
    def add_point_cloud(self, point_cloud: PointCloud) -> str:
        ...
        Returns:
            uuid.UUID: A unique identifier assigned to the newly added point cloud.
        """
        ...
        # Store the point cloud data along with its metadata
        self._point_clouds[point_cloud_uuid] = point_cloud
        self._notify_changes()
        return point_cloud_uuid

    def remove_point_cloud(self, point_cloud_uuid: uuid.UUID) -> bool:
        """
    ...
    def get_point_cloud(self, point_cloud_uuid: uuid.UUID) -> Optional[PointCloud]:
        """
        Retrieves a `PointCloud` instance by its UUID.

        Args:
            point_cloud_uuid (uuid.UUID): The unique identifier for the `PointCloud` to retrieve.
        ...
        if point_cloud:
            ...
            print(f"Point cloud '{point_cloud_uuid}' not found.")
            return None
    ...
    def _notify_changes(self):
        ...
        if current_keys != self._previous_keys:
            # Emit the signal with old and new keys
