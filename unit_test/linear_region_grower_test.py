...
def test_the_offer_ahead_leaves_the_clutter_out():
    ...
    ground = np.column_stack([rng.uniform(-5, 35, 20000), rng.uniform(-8, 8, 20000),
                              rng.normal(0, 0.03, 20000)])
    tree = np.column_stack([rng.normal(13, 1.2, 6000), rng.normal(4, 1.2, 6000),
                            rng.uniform(0, 9, 6000)])
    points = np.vstack([cable + rng.normal(0, 0.01, cable.shape), ground, tree])
