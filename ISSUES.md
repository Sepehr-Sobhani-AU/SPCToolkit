...
- ~~For some reason subtracting ground branch from estimated normal branch is extremely slow. If I'm not wrong It should be a mask array applying on the estimate normal branch.~~
  ...
  - ~~***OPEN (new, separate task):*** Change the background color of branches whose datanode data length ≠ their root ancestor's point count.~~ **Done (revised twice):** *length ≠ root* was the wrong signal; a binary red flag was also wrong — subtract speed isn't a property of one branch but of **two branches sharing an index space**. Now every branch is **tinted by its effective root** (`get_node_effective_root` — topmost ancestor reachable through a pure subset/identity chain). **Same colour ⇒ logical ops stay fast (index space); different colour ⇒ subtract would fall back to slow coordinate matching.** A branch re-rooted through a materialised `point_cloud` (merge / duplicate-to-root) becomes its own colour group; a second loaded cloud gets its own colour too. Palette of 10 pastels, assigned in order of appearance, roots included (they head their group).
