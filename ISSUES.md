...
- ~~For some reason subtracting ground branch from estimated normal branch is extremely slow. If I'm not wrong It should be a mask array applying on the estimate normal branch.~~
  ...
  - ~~***OPEN (new, separate task):*** Change the background color of branches whose datanode data length ≠ their root ancestor's point count.~~ **Done (revised):** *length ≠ root* was the wrong signal — a 29.6M `separate_selected_clusters` branch still subtracts instantly because the lineage composes its masks back to root indices. So the highlight now flags **child branches that are NOT index-mappable to their root** (`is_branch_composable_to_root` — lineage isn't a pure subset/identity chain, e.g. re-rooted through a materialised `point_cloud`), meaning logical ops would fall back to slow coordinate matching. **Light red**, and **roots are never highlighted**. On a clean mask/cluster tree nothing is flagged (matches "everything works fast").
